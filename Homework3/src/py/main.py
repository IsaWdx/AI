...
def _parse_args():
  ...
  parser.add_argument('--sample-file', help='')
  if len(sys.argv) == 1:
    parser.print_help()
    sys.exit(1)
...
def configure_theano():
  ...
  theano.config.cuda.enabled = False
...
def evaluate(name, model, dataset, domain=None, sample_file='sample'):
  """Evaluate the model. """
  in_vocabulary = model.in_vocabulary
  out_vocabulary = model.out_vocabulary
  ...
  for i, ex in enumerate(dataset):
    ...
    print >> f_out, ex.x_str
    print >> f_out, ex.y_str
    print >> f_out, y_pred_str
    print >> f_out
...
def evaluate_train(model, train_data, domain=None, sample_file='sample'):
  print >> sys.stderr, 'Evaluating on training data...'
  print 'Training data:'
  evaluate('train', model, train_data, domain=domain, sample_file=sample_file)

def evaluate_dev(model, dev_raw, domain=None, sample_file='sample'):
  print >> sys.stderr, 'Evaluating on dev data...'
  dev_model = update_model(model, dev_raw)
  dev_data = preprocess_data(dev_model, dev_raw)
  print 'Dev data:'
  evaluate('dev', dev_model, dev_data, domain=domain, sample_file=sample_file)
...
def run():
  ...
  if train_raw:
    evaluate_train(model, train_data, domain=domain, sample_file='%s.train' % OPTIONS.sample_file)
  if dev_raw:
    evaluate_dev(model, dev_raw, domain=domain, sample_file='%s.dev' % OPTIONS.sample_file)
